/**
 * LibraryCard - 라이브러리 아이템 카드 컴포넌트
 * 
 * 주요 역할:
 * 1. 클립, 프로젝트, 업로드 영상의 통합된 카드 UI 제공
 * 2. 아이템 타입별 특화된 시각적 표현 및 메타데이터 표시
 * 3. 선택 상태 관리 및 다중 선택 순서 표시
 * 4. 호버 시 영상 미리보기 및 프리로딩 지원
 * 
 * 핵심 특징:
 * - 3가지 아이템 타입(clip, project, upload)에 대한 통합 인터페이스
 * - HoverVideo 컴포넌트 연동으로 부드러운 영상 프리뷰
 * - 선택 순서 표시로 다중 선택 시 사용자 경험 향상
 * - 현재 프로젝트 하이라이트 및 테마 커스터마이제이션
 * - React.memo로 렌더링 최적화
 * 
 * 주의사항:
 * - 아이템 타입에 따라 다른 메타데이터 접근 방식 필요
 * - 영상 프리로딩은 성능과 대역폭을 고려하여 제한적 사용
 * - 선택 상태 변경 시 상위 컴포넌트와 동기화 중요
 */
'use client';
