/**
 * LibrarySection - 라이브러리 섹션 컴포넌트
 * 
 * 주요 역할:
 * 1. 특정 타입의 라이브러리 아이템들을 섹션 단위로 그룹화하여 표시
 * 2. 무한 스크롤 기능과 연동하여 대량 데이터 처리
 * 3. 로딩, 에러 상태에 대한 적절한 UI 피드백 제공
 * 4. 영상 프리로딩 최적화로 사용자 경험 향상
 * 
 * 핵심 특징:
 * - 섹션별 제목과 아이콘으로 명확한 구분 제공
 * - InfiniteScrollLoader 연동으로 끊김 없는 스크롤 경험
 * - LibraryCard 컴포넌트 통합으로 일관된 아이템 표시
 * - 선택적 영상 프리로딩으로 성능 최적화
 * - React.memo로 불필요한 리렌더링 방지
 * 
 * 주의사항:
 * - 무한 스크롤 트리거 시점의 정확한 계산 필요
 * - 대량 아이템 렌더링 시 가상화 고려
 * - 에러 상태에서도 사용자가 재시도할 수 있는 UI 제공
 */
'use client';
