  ...
  try {
      try {
        ...
        if (process.env.NODE_ENV === 'development') {
          console.log('Image brush request from user:', { id: user.id, email: user.email });
        }
    ...
    if (!response.ok) {
      ...
      return NextResponse.json(
        ...
        console.log('Image brush processing completed successfully', {
          user_id: user.id,
          mode: body.mode,
        ...
        // 에러 로깅
        const duration = Date.now() - startTime;
        const errorType = error instanceof Error && (error.name === 'AbortError' || error.name === 'TimeoutError') 
          ? 'timeout_error' 
          : 'image_processing_error';
        
        console.error('Image brush processing failed', {
