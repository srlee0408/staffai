  ...
  try {
      ...
      try {
        ...
        if (process.env.NODE_ENV === 'development') {
          console.log('Sound generation request from user:', { id: user.id, email: user.email });
        }
    ...
    console.log('Sound generation request initiated successfully', {
      user_id: user.id,
      group_id: groupId,
    ...
    return NextResponse.json({
      ...
      // 에러 로깅
      const duration = Date.now() - startTime;
      console.error('Sound generation request failed', {
